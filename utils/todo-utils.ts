 ...
 */
export const formatEta = (timestamp: number, t: (key: string) => string): EtaFormat => {
  ...
  if (isToday(date)) {
    if (minutesRemaining <= 0) {
      ...
      etaText =
        minutesRemaining === 1
          ? `${t("etaToday")}, ${t("minutesLeft").replace("{minutes}", minutesRemaining.toString())}`
          : `${t("etaToday")}, ${t("minutesLeftPlural").replace("{minutes}", minutesRemaining.toString())}`
    } else {
      const hours = Math.floor(minutesRemaining / 60)
      const mins = minutesRemaining % 60
      etaText = mins
        ? `${t("etaToday")}, ${t("hoursMinutesLeft").replace("{hours}", hours.toString()).replace("{minutes}", mins.toString())}`
        : `${t("etaToday")}, ${t("hoursLeft").replace("{hours}", hours.toString())}`
    }
